...
export default function DeepfakeVideoDetection() {
  ...
  const handleFile = (file: File) => {
    ...
    // Size check (e.g., 50MB limit for demo)
  ...
  const handleAnalyze = async () => {
    ...
    try {
      // Create FormData
      // We need the file object. Let's assume we modify handleFile to state.
      // Similar to Image detection, I'll add selectedFile state.
      if (!selectedFile) {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/scan/video`,
        {
          method: "POST",
          headers: {
      ...
      setResult({
        ...
        details: {
          ...
          blinkingPatterns: data.blinking_patterns,
        },
      });
      ...
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16 space-y-4">
          ...
          <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight text-foreground">
            Video Authenticity Scanner
          </h1>
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
            Upload videos to detect AI-generated manipulations, face swaps, and
            lip-sync anomalies
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto mb-20 text-center">
          <div className="flex flex-col items-center space-y-3">
            ...
            <p className="text-sm text-muted-foreground max-w-xs">
              Frame-by-frame inspection for digital artifacts
            </p>
          </div>
          <div className="flex flex-col items-center space-y-3">
            ...
            <p className="text-sm text-muted-foreground max-w-xs">
              Optimized algorithms for fast video processing
            </p>
          </div>
          <div className="flex flex-col items-center space-y-3">
            ...
            <p className="text-sm text-muted-foreground max-w-xs">
              Detects mismatch between lip movement and speech
            </p>
          </div>
        </div>
          ...
          <Card className="flex-1 w-full shadow-xl border-slate-200 dark:border-slate-800 bg-white dark:bg-card">
            <CardHeader className="text-center border-b border-border/50 pb-8 pt-8 px-6 bg-slate-50/50 dark:bg-slate-900/20">
              ...
              <CardDescription className="text-base">
                Upload a video to check for AI manipulation
              </CardDescription>
            </CardHeader>
                ...
                {selectedVideo ? (
                  <div className="relative w-full h-full flex flex-col items-center">
                    <video
                      src={selectedVideo}
                      controls
                      className="max-h-[300px] w-full rounded-lg shadow-md bg-black"
                    />
                    <Button
                      variant="destructive"
                      size="sm"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4">
                    <div className="p-3 bg-secondary/20 rounded-lg border border-border/50">
                      ...
                      <p className="text-sm font-semibold">
                        {result.details.frameConsistency}
                      </p>
                    </div>
                    <div className="p-3 bg-secondary/20 rounded-lg border border-border/50">
                      <p className="text-xs font-medium text-muted-foreground mb-1">
                    ...
                    <div className="p-3 bg-secondary/20 rounded-lg border border-border/50">
                      ...
                      <p className="text-sm font-semibold">
                        {result.details.blinkingPatterns}
                      </p>
                    </div>
                  </div>
